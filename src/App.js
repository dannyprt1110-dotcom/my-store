...
function AdminBrand({ brand, setBrand, showToast }) {
  ...
  const save = () => { setBrand(local); showToast("品牌設定已儲存！"); };
  return (
    <div>
...
function AdminProducts({ products, setProducts, showToast }) {
  ...
  const save = () => {
    if (editing === "new") setProducts((ps) => [...ps, form]);
    else setProducts((ps) => ps.map((p) => p.id === editing ? form : p));
...
function AdminSizes({ sizes, setSizes, showToast }) {
  ...
  const save = () => { setSizes(local); showToast("尺寸表已儲存！"); };
  return (
    <div>
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", top: 16, right: 16, zIndex: 9999, display: "flex", background: "#111", border: "1px solid #222", borderRadius: 6, overflow: "hidden", fontFamily: "'Bebas Neue',sans-serif" }}>
        {["admin", "store"].map((m) => (
          <button key={m} onClick={() => setMode(m)} style={{ padding: "8px 20px", fontSize: ".85rem", letterSpacing: ".15em", background: mode === m ? "#fff" : "transparent", color: mode === m ? "#000" : "#666", border: "none", cursor: "pointer", transition: "all .2s" }}>
            {m === "admin" ? "⚙ ADMIN" : "👁 PREVIEW"}
          </button>
        ))}
      </div>
      {mode === "admin"
        ? <Admin brand={brand} setBrand={setBrand} products={products} setProducts={setProducts} sizes={sizes} setSizes={setSizes} onPreview={() => setMode("store")} />
